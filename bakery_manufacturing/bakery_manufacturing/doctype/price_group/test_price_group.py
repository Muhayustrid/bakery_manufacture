...
IGNORE_TEST_RECORD_DEPENDENCIES = ["Item", "Company", "Warehouse", "POS Profile"]


class TestPriceGroup(IntegrationTestCase):
	def setUp(self):
		super().setUp()
	...
	def test_save_creates_price_list_and_item_prices(self):
		...
		ip2 = frappe.db.get_value(
			"Item Price", {"item_code": item2, "price_list": "PG-Test Group A"}, "price_list_rate"
		)
	...
	def test_update_rate_syncs_item_price(self):
		...
		ip_rate = frappe.db.get_value(
			"Item Price", {"item_code": item1, "price_list": "PG-Test Group A"}, "price_list_rate"
		)
	...
	def test_remove_item_deletes_item_price(self):
		...
		pg.insert(ignore_permissions=True)
		self.assertTrue(frappe.db.exists("Item Price", {"item_code": item2, "price_list": "PG-Test Group A"}))
		...
		self.assertFalse(
			frappe.db.exists("Item Price", {"item_code": item2, "price_list": "PG-Test Group A"})
		)
		# Item 1 still exists
		self.assertTrue(frappe.db.exists("Item Price", {"item_code": item1, "price_list": "PG-Test Group A"}))
	...
	def _ensure_warehouse(self, name=None, company=None):
		...
		if frappe.db.exists(
			"Warehouse", {"warehouse_name": wh_name, "company": company or self._get_default_company()}
		):
			return frappe.db.get_value(
				"Warehouse",
				{"warehouse_name": wh_name, "company": company or self._get_default_company()},
				"name",
			)
		wh = frappe.get_doc(
			{
	...
	def _non_group_account(self, company, account):
		"""Return ``account`` only if it is a non-group Account of ``company``.

		Company defaults are plain Link fields with no company guarantee, so they are
	...
	def _ensure_pos_profile(self):
		try:
			...
			existing = frappe.db.get_value(
				"POS Profile", {"company": company, "warehouse": warehouse}, "name"
			)
			if existing:
				return existing
			# Resolve required fields — fall back gracefully if missing.
			# POS Profile.write_off_account is mandatory but this fixture never posts a
			# write-off, so the ladder goes from semantically exact to merely valid:
			# the account_type, then the Company default, then a name match, and only
			# last a Round Off account as a stand-in. Every step stays scoped to this
			# company and to non-group accounts. If none resolve, the test skips rather
			# than inventing an account on a shared site.
			write_off_account = (
				frappe.db.get_value(
					"Account", {"company": company, "account_type": "Write Off", "is_group": 0}, "name"
				)
				or self._non_group_account(
					company, frappe.db.get_value("Company", company, "write_off_account")
				)
				or frappe.db.get_value(
					"Account", {"company": company, "name": ["like", "%Write Off%"], "is_group": 0}, "name"
				)
				or self._non_group_account(
					company, frappe.db.get_value("Company", company, "round_off_account")
				)
				or frappe.db.get_value(
